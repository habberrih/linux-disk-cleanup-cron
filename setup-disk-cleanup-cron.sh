...
main() {
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      -t|--threshold-gb)
        THRESHOLD_GB="$2"; shift 2;;
      -s|--schedule)
        SCHEDULE="$2"; shift 2;;
      -p|--path)
        TARGET_PATH="$2"; shift 2;;
      --enable-docker-prune)
        ENABLE_DOCKER_PRUNE=1; shift;;
      --docker-prune-hours)
        DOCKER_PRUNE_HOURS="$2"; shift 2;;
      --docker-prune-include-volumes)
        DOCKER_PRUNE_INCLUDE_VOLUMES=1; shift;;
      --cleanup-script)
        CLEANUP_SRC_OVERRIDE="$2"; shift 2;;
      --cron-only)
        CRON_ONLY=1; shift;;
      --uninstall)
  ...
  local ENV_PREFIX="THRESHOLD_GB=${THRESHOLD_GB} TARGET_PATH=${TARGET_PATH}"
  ...
  if command -v logger >/dev/null 2>&1; then
    ...
    # Fallback path (common on many distros); cron line will still work if logger exists later
    LOGGER_BIN="/usr/bin/logger"
  fi

  # Build cron line. Redirect output to syslog to avoid local mail spools.
  local CRON_LINE
